# SA-DINO: Semantic-Aware DINO for periapical periodontitis detection
#
# Based on the best-performing medical_pipeline config (mAP=0.480),
# with deep tooth semantic mask integration:
# - ToothEmbeddingEncoder: per-tooth-ID learnable embeddings
# - Improved MaskGuidedFeatureModulation with negative-bias gate init
# - SemanticCrossAttention on encoder output memory (3 layers)
# - Auxiliary tooth segmentation head (CE loss, weight=0.4)
#
# Training:
#   CUDA_VISIBLE_DEVICES=0 python tools/train.py \
#     projects/dental_dino/configs/dino-4scale_r50_8xb2-12e_perio_sadino.py
...
# Keep the medical_pipeline's proven augmentation strategy:
# horizontal flip only, no random crop, multi-scale resize
# Added: with_seg=True to load tooth semantic mask
train_pipeline = [
...
# Optimizer: same as medical_pipeline base (from dino coco config)
# backbone 0.1x lr; tooth prior modules get default lr (effectively 10x vs backbone)
optim_wrapper = dict(
    type='OptimWrapper',
    optimizer=dict(
        type='AdamW',
