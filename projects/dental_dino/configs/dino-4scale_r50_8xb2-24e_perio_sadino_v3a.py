# SA-DINO v3a: Tooth-RoI Local Refinement Head
#
# Builds on SA-DINO v2 with:
# - High-res C2 feature extraction (backbone out_indices=(0,1,2,3))
# - RoI Align + ModulatedDeformConv refinement head
# - Tooth-aware RoI expansion using anatomical mask context
# - Delta + IoU prediction for cascade-style box refinement
#
# Training:
#   CUDA_VISIBLE_DEVICES=2 bash projects/dental_dino/scripts/run_train.sh \
#     projects/dental_dino/configs/dino-4scale_r50_8xb2-24e_perio_sadino_v3a.py \
#     --work-dir work_dirs/dino-4scale_r50_8xb2-24e_perio_sadino_v3a
...
model = dict(
    type='SADINO',
    # backbone: add C2 output (out_indices=(0,1,2,3)) for high-res refinement
    backbone=dict(
        out_indices=(0, 1, 2, 3),
        frozen_stages=1,
    ),
    # neck: only takes C3,C4,C5 (3 channels) — C2 is handled separately
    neck=dict(
